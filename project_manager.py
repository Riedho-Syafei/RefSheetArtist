...
class CharacterProfile:
    ...
    name: str
    base_prompt: str
    # Which GenerationBackend this project was generated with (e.g. "flux",
    # "sdxl_furry"). Defaults to "flux" so profile.json files saved before
    # this field existed still load correctly via CharacterProfile(**data) —
    # the key is simply absent and the default fills in.
    backend: str = "flux"
...
if __name__ == "__main__":
    ...
    assert loaded_profile.backend == "flux"
    ...
    # Simulate a project saved before `backend` existed on CharacterProfile,
    # to confirm old projects still load without a migration step.
    legacy_dir = manager.project_dir("Legacy Character")
    legacy_dir.mkdir(parents=True, exist_ok=True)
    (legacy_dir / PROFILE_FILENAME).write_text(json.dumps({
        "name": "Legacy Character",
        "base_prompt": "a legacy test character",
    }))
    (legacy_dir / MANIFEST_FILENAME).write_text(json.dumps({}))
    legacy_profile, _ = manager.load_project("Legacy Character")
    assert legacy_profile.backend == "flux", "old profile.json without 'backend' should default to 'flux'"
    print("Legacy profile.json (no backend key) loaded OK, defaulted to:", legacy_profile.backend)
